...
class Controller {
	 ...
	 */ onFormSubmit(ev) {
        switch(ev.target.id){
            case "bmr-calculator":
                (0, _modelDefault.default).state.person = (0, _viewDefault.default).getBMRValues(ev.target);
...
class View {
	 ...
	 * @return object
	 */ getBMRValues(form) {
        const weightVal = form.querySelector("#weight"), heightFtVal = form.querySelector("#height--ft"), heightInVal = form.querySelector("#height--in"), ageVal = form.querySelector("#age"), genderOptions = form.querySelectorAll('#gender input[type="radio"]');
        ...
        genderOptions.forEach((el, i)=>{
            if (el.checked) person.gender = genderOptions[i].value;
        });
        return person;
    }
     #getOptionsValue(el) {
	 ...
	 * @return object
	 */ getModsValues(form) {
        const activityVal = form.querySelector("#tdee"), deficitVal = form.querySelector("#deficit");
        const modifiers = {
    ...
    /** Toggles '.inactive' class for each form to clarify UI. */  #toggleStyle(forms) {
        forms.forEach((form)=>form.classList.toggle("inactive"));
    }
    /** Enables form field inputs */  #enableForm(form1) {
